...
class Darknet(nn.Module):
    ...
    def _create_conv_layers(self, architecture):
        ...
        for x in architecture:
            if type(x) == tuple:  
                layers.append(CNNBlock(in_channels=in_channels, kernel_size=x[0], out_channels=x[1], stride=x[2], padding=x[3]))
                in_channels = x[1]
                ...
                for _ in range(x[2]):
                    # Conv1
                    layers.append(CNNBlock(in_channels=in_channels, kernel_size=x[0][0], out_channels=x[0][1], stride=x[0][2], padding=x[0][3]))
                    in_channels = x[0][1]
                    
                    # Conv2
                    layers.append(CNNBlock(in_channels=in_channels, kernel_size=x[1][0], out_channels=x[1][1], stride=x[1][2], padding=x[1][3]))
                    in_channels = x[1][1]
