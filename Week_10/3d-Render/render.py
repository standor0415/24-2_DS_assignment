...
def scaled_vertices(face, vertices, width, height):
    v0 = [int((x + 1) * (width // 2)) for x in vertices[face[0][0] - 1]]
    v1 = [int((x + 1) * (width // 2)) for x in vertices[face[1][0] - 1]]
    v2 = [int((x + 1) * (width // 2)) for x in vertices[face[2][0] - 1]]
...
# --- Triangle Drawing ---
def draw_triangle(pixels, vertices, face, width, height):
    v0, v1, v2 = scaled_vertices(face, vertices, width, height)
...
def shade_triangle_texture(
    pixels, vertices, texture_vertices, face, z_buffer, texture_array, tex_dim, width, height
):
    # We scale the vertices to the screen size first so everything
    # between -1 and 1 neatly maps onto the image.
    v0, v1, v2 = scaled_vertices(face, vertices, width, height)  # width, height 추가
...
def shade_triangle(pixels, vertices, face, z_buffer, width, height):
    # We scale the vertices to the screen size first so everything
    # between -1 and 1 neatly maps onto the image.
    v0, v1, v2 = scaled_vertices(face, vertices, width, height)
    min_x, max_x, min_y, max_y = bounding_box(v0, v1, v2)
...
def render_shaded(pixels, vertices, texture_vertices, faces, texture_array, texture_dim, width, height):
    z_buffer = np.zeros((width, height))
    z_buffer.fill(-999)
    for face in faces:
        if texture_array is not None:  # When TGA exists
            shade_triangle_texture(
                pixels, vertices, texture_vertices, face, z_buffer, texture_array, texture_dim, width, height
            )
        else:
            shade_triangle(pixels, vertices, face, z_buffer, width, height)
